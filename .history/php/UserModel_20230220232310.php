...
class UserModel extends DatabaseConnection
{
    ...
    function insertIntoContact()
    {
        $query = "INSERT INTO contact(username, password, email, age,usetype) VALUES (?,?,?,?,?)";
        $stm = $this->conn->prepare($query);

        if ($stm->execute([$this->username, $this->password, $this->email, $this->age, $this->usetype])) {
            echo "<script>alert('records added successfully');</script>";
            echo "<script>window.location.href = 'index.php';</script>";
        } else {
